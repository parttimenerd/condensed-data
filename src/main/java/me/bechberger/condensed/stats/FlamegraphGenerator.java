 ...
 *
 * <p>Inspired by <a
 * href="https://github.com/parttimenerd/tiny-profiler/blob/main/src/main/java/me/bechberger/Store.java">Tiny
 * Profiler</a>
 */
        ...
        aggregated.entrySet().stream()
                ...
                .forEach(
                        entry -> {
                            ...
                            double overallBytesPerEvent =
                                    stats.count > 0
                                            ? (double) stats.overallBytes / stats.count
                                            : 0.0;
                            double percentageOfTotal =
                                    totalBytes > 0
                                            ? (double) stats.directBytes * 100.0 / totalBytes
                                            : 0.0;
                            double overallPercentageOfTotal =
                                    totalBytes > 0
                                            ? (double) stats.overallBytes * 100.0 / totalBytes
                                            : 0.0;
                            s.printf(
                                    "%-40s | %13d | %13d | %11d | %20.2f | %20.2f%% | %20.2f%%%n",
                                    typeName,
                                    stats.directBytes,
                                    stats.overallBytes,
                                    stats.count,
                                    ...
                                    percentageOfTotal,
                                    overallPercentageOfTotal);
                        });
    }

    /**
     * Returns a JSONArray with table data sorted by overall bytes written per event type. Each
     * entry contains: typeName, directBytes, overallBytes, count, overallBytesPerMessage,
     * percentageOfTotal
     *
