 ...
 *
 * <p>{@code eventType} is a synthetic pseudo-field: every event exposes {@code eventType.label} (the
 * type's human label, e.g. "Young Garbage Collection") and {@code eventType.name} (the type name,
 * e.g. "jdk.YoungGarbageCollection"). These are not stored fields; they come from the event's own
 * type metadata, mirroring {@code jfr view}'s {@code EventType} accessor.
 */
final class FieldResolver {
    ...
    static Object resolve(ReadStruct event, List<String> parts) {
        ...
        for (int i = 0; i < parts.size(); i++) {
            ...
            // even though the stored struct only has {truncated, frames[]}. Resolve it to frames[0].
            ...
            // Synthetic StackTrace.topNotInitFrame: jfr's accessor for the topmost frame whose method
            // is NOT a constructor/initializer (<init>/<clinit>). Used by exception-by-site, where the
            // top frames are the Throwable subclass constructor chain (Error.<init>, …); the "site" is
            // the first real caller below them (e.g. MethodHandleNatives.resolve). Falls back to the
            // first frame if every frame is an init frame.
            if ("topNotInitFrame".equals(part) && !s.hasField("topNotInitFrame") && s.hasField("frames")) {
                Object frames = s.get("frames");
                if (frames instanceof List<?> list && !list.isEmpty()) {
    ...
    /**
     * The first frame whose method name is not {@code <init>} or {@code <clinit>}, or the first frame
     * if all are initializers. A frame is a struct with a {@code method} struct that has a {@code name}.
     */
    private static Object firstNotInitFrame(List<?> frames) {
        for (Object f : frames) {
            ...
            String name = method != null && method.hasField("name") ? asString(method.get("name")) : null;
    ...
    /**
     * Resolve the synthetic {@code eventType} pseudo-field's trailing accessor. {@code .label} → the
     * type's human label; {@code .name} → the fully-qualified type name; bare {@code eventType} → the
     * label (jfr renders the type by its label).
     */
     ...
     * {@code ["label", "description"]}; we pull the first element. Falls back to the type name if the
     * description is absent or not in that shape.
     */
